...
"""CLI script to generate KML files from Business Discovery results.

Exports business discovery search results into a Keyhole Markup Language (KML) file
stored in the current working directory for visualization in Google Earth, Maps, or GIS software.

Examples:
    1. Live Search & KML Export:
    ...
    2. Convert Existing JSON Result to KML:
       uv run skills/business-discovery/scripts/export_kml.py \\
           --input-json "results.json" \\
           --output "businesses.kml"
"""
...
def create_parser() -> argparse.ArgumentParser:
    """Construct CLI argument parser for KML export tool."""
    parser = argparse.ArgumentParser(
        description="Generate KML files from Business Discovery results and save to the current working directory."
    )
    ...
    output_group.add_argument(
        ...
        type=str,
        default=None,
        help="Output KML filename or path (saved in the current working directory by default).",
    )
...
def main(args_list: Optional[list[str]] = None) -> int:
    """Main CLI execution entry point for KML exporter.
    ...
    try:
        if args.input_json:
            ...
            result = KMLExporter.load_result_from_json(args.input_json)
        else:
            ...
            console.print(
                f"[bold cyan]🔍 Executing Business Discovery search for KML export...[/bold cyan]\n"
        ...
        # Export discovery result to KML in the current working directory
        cwd = Path.cwd()
        kml_path = KMLExporter.export_to_kml(
            result=result,
        ...
        console.print(
            f"\n[bold green]✓ KML output generated successfully![/bold green]\n"
            f"  • [bold white]Placemarks Exported:[/bold white] [cyan]{len(result.businesses)}[/cyan]\n"
            f"  • [bold white]KML File Location:[/bold white] [green]{kml_path}[/green]"
        )
        return 0
        ...
        return 1
    except KMLExporterError as err:
        console.print(f"[bold red]KML Exporter Error:[/bold red] {err}")
        return 1
